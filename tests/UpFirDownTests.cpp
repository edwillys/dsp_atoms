                        ...
                        cfloat32_t eps, cint32_t blocksize, bool_t verbose)
{
    ...
    int32_t out_len;
    if (0 == blocksize)
        out_len = ufp.getLenOut(us, ds, (int32_t)in[0].size());
    else
        out_len = blocksize;
    ...
    {
        out = ufp.apply(it->data(), (int32_t)it->size());
                                  ...
                                  cfloat32_t amplitude, cfloat32_t dutycycle)
{
    ...
    int32_t iter = (int32_t)ceil(len / period);
    std::vector<float32_t> ret = std::vector<float32_t>(iter * (int32_t)period);

    int32_t ind = 0;
    for (auto i = 0; i < iter; i++)
    {
        int32_t npos = (int32_t)(period * dutycycle);
        int32_t nneg = (int32_t)(period - npos);
